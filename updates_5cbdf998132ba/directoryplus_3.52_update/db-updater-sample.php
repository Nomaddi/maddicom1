...
/*
NEW TEXT STRINGS FOR v.3.52
also must include in
	directoryplus.sql,
	lang_xx.sql.
DO NOT INCLUDE IN directoryplus_3.52_update.sql
*/

// check if a new txt_ string for v.3.52 exists
$query = "SELECT count(*) AS c FROM language WHERE `section`='admin' AND var_name='txt_auto_approve_listing'";
$stmt  = $conn->prepare($query);
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);

if($row['c'] < 1) {
	// removed txt strings in v.3.52
	$query = "DELETE FROM `language` WHERE
			(`section`='admin' AND var_name='txt_timezone_explain' AND template='settings')
			OR (`section`='admin' AND var_name='txt_by_date' AND template='users')
			OR (`section`='admin' AND var_name='txt_by_email' AND template='users')
			OR (`section`='admin' AND var_name='txt_by_name' AND template='users')
			OR (`section`='admin' AND var_name='txt_by_date' AND template='users-trash')
			OR (`section`='admin' AND var_name='txt_by_email' AND template='users-trash')
			OR (`section`='admin' AND var_name='txt_by_name' AND template='users-trash')
			;";
	$stmt  = $conn->prepare($query);
	$stmt->execute();
	...
	foreach($lang_arr as $v) {
		$query = "INSERT INTO `language` (`lang`, `section`, `template`, `var_name`, `translated`) VALUES
		('$v', 'admin', 'settings', 'txt_auto_approve_listing', 'Auto-approve listing'),
		('$v', 'admin', 'settings', 'txt_smtp_encryption', 'SMTP encryption'),
		('$v', 'admin', 'settings', 'txt_create_webhook', 'Create webhook'),
		('$v', 'admin', 'settings', 'txt_create_test_webhook', 'Create test webhook'),
		('$v', 'admin', 'settings', 'txt_create_live_webhook', 'Create live webhook'),
		('$v', 'admin', 'settings', 'txt_none', 'None'),
		('$v', 'admin', 'settings', 'txt_cents', 'Cents')
		;";
		$stmt  = $conn->prepare($query);
		$stmt->execute();
	}
