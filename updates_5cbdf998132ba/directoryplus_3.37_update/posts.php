...
if(empty($term)) {
	...
	$query = "SELECT COUNT(page_id) AS c FROM pages WHERE page_status > -1 AND MATCH(page_title, meta_desc, page_contents) AGAINST(:term IN BOOLEAN MODE)";
	$stmt = $conn->prepare($query);
