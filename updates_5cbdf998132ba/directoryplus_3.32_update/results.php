...
if($cur_cat_parent_id == 0 or true) {
...
if(!empty($custom_fields)) {
	...
	foreach($custom_fields as $k => $v) {
		...
		*/
			...
			// open parenthesis
			if(!empty($field_value['from']) && !empty($field_value['to'])) {
				$query_where .= "(";
			}
...
if($total_rows > 0) {
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		if(!empty($this_logo) && file_exists($pic_basepath . '/main-pic/' . substr($this_logo, 0, 2) . '/' . $this_logo)) {
			$this_logo_url = $pic_baseurl . '/main-pic/' . substr($this_logo, 0, 2) . '/' . $this_logo;
		}
