	...
	if(empty($in_str)) {
		if($total_rows > 0) {
			...
			$query = "SELECT
				...
				$having
				$max_dist_having
				ORDER BY p.feat DESC, pt.plan_priority DESC, $sort p.place_id DESC
...
if($total_rows > 0) {
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$list_items[$this_place_id] = array(
		...
		$listings_ids[$this_place_id] = $this_place_id;
	}
}
...
Ratings
--------------------------------------------------*/
$listings_ids_str = implode(",", $listings_ids);

if(!empty($list_items)) {
	$query = "SELECT place_id, AVG(reviews.rating) AS avg_rating
				FROM reviews
				WHERE place_id IN($listings_ids_str)
				GROUP BY place_id";
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$list_items[$place_id]['rating'] = $avg_rating;
	}
}
...
if(empty($list_items)) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// populate array of listings ids for this results set
		$latest_listings_ids[$this_place_id] = $this_place_id;
	}
}
...
if(!empty($latest_listings)) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$latest_listings[$place_id]['rating'] = $avg_rating;
	}
}
