...
$cfg_use_select2              = !empty($_POST['cfg_use_select2'             ]) ? $_POST['cfg_use_select2'             ] : 0;
...
$mail_after_post              = !empty($_POST['mail_after_post'             ]) ? $_POST['mail_after_post'             ] : 0;
$maintenance_mode             = !empty($_POST['maintenance_mode'            ]) ? $_POST['maintenance_mode'            ] : 0;
...
$user_created_notify          = !empty($_POST['user_created_notify'         ]) ? $_POST['user_created_notify'         ] : 0;
...
//$mail_after_post              = !empty($mail_after_post             ) ? $mail_after_post              : 1;
...
//$maintenance_mode             = !empty($maintenance_mode            ) ? $maintenance_mode             : 0;
...
//$user_created_notify          = !empty($user_created_notify         ) ? $user_created_notify          : 1;
...
//$cfg_use_select2              = !empty($cfg_use_select2             ) ? $cfg_use_select2              : 0;
...
try {
	...
	$query = "ALTER TABLE config DROP id;";
	$stmt = $conn->prepare($query);
	$stmt->execute();

	$query = "ALTER TABLE config ADD id int UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST;";
	$stmt = $conn->prepare($query);
	$stmt->execute();
