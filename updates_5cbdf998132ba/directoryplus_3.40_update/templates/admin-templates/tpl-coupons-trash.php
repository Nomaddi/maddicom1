					...
					<table class="table admin-table">
						<tr>
							<th class="text-nowrap">
								...
								?>
								<a href="<?= $baseurl ?>/admin/coupons-trash?sort=<?= $sort_param ?>">
								<?= $txt_id ?>
								<?php
							...
							<th class="text-nowrap">
								...
								<?php
								$sort_icon = '<i class="las la-sort"></i>';
								if($sort == 'expire') $sort_icon = '<i class="las la-sort-up"></i>';
								if($sort == 'expire-desc') $sort_icon = '<i class="las la-sort-down"></i>';
						...
						foreach($coupons_arr as $k => $v) {
							...
							?>
							<tr id="tr-coupon-<?= $coupon_id ?>">
								<td class="text-nowrap shrink"><?= $coupon_id ?></td>
								<td>
									<a href="<?= $coupon_link ?>"><?= $coupon_title ?></a>
									<br><small><?= $place_name ?></small>
								</td>
								<td class="text-nowrap shrink"><?= $coupon_created ?></td>
								<td class="text-nowrap shrink"><?= $coupon_expire ?></td>
...
<!-- Remove coupon modal -->
<div id="remove-coupon-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-title-1">
...
(function(){
	// on show modal
	$('#remove-coupon-modal').on('show.bs.modal', function(e) {
		// vars
		var button = $(e.relatedTarget);
		var coupon_id = button.data('coupon-id');

		// add coupon id value to button
		$('#remove-coupon-modal .remove-coupon').attr('data-coupon-id', coupon_id);
	});
	...
	$('.remove-coupon').on('click', function(e){
		...
		// vars
		var coupon_id = $('.remove-coupon').attr('data-coupon-id');
