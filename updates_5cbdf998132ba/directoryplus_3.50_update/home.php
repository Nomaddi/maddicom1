...
$query = "SELECT
	p.place_id, p.userid, p.place_name, p.city_id, p.description, p.short_desc, p.address, p.feat, p.slug AS place_slug, p.logo,
	ph.dir, ph.filename,
	c.city_name, c.slug,
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	$feat_logo           = !empty($row['logo'       ]) ? $row['logo'       ] : '';
	...
	if(!empty($feat_logo)) {
		if(file_exists($pic_basepath . '/logo/' . substr($feat_logo, 0, 2) . '/' . $feat_logo)) {
			$latest_photo_url = $pic_baseurl . '/logo/' . substr($feat_logo, 0, 2) . '/' . $feat_logo;
		}
	}
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	$latest_logo           = !empty($row['logo'       ]) ? $row['logo'       ] : '';
	...
	if(!empty($latest_logo)) {
		if(file_exists($pic_basepath . '/logo/' . substr($latest_logo, 0, 2) . '/' . $latest_logo)) {
			$latest_photo_url = $pic_baseurl . '/logo/' . substr($latest_logo, 0, 2) . '/' . $latest_logo;
		}
	}
