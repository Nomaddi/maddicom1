...
if($row['c'] == 0) {
	...
	if(basename($_SERVER['SCRIPT_NAME']) == 'install.php' || basename($_SERVER['SCRIPT_NAME']) == 'process-install.php') {
		...
		die("Directoryplus is not installed. <a href='$baseurl/install.php'>Install</a>");
	}
}
...
$cfg_auto_approve_listing = 1;

// enable sitemaps
$cfg_enable_sitemaps = 1;

// enable reviews
$cfg_enable_reviews = 1;

// enable coupons
$cfg_enable_coupons = 1;
...
$cfg_cur_without_cents = 0;

/*--------------------------------------------------
Vars not in admin area (currently only hardcoded in this common.inc.php file)
--------------------------------------------------*/
...
$cfg_show_maps_on_listings = false;
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	switch ($row['property']) {
		...
		case 'cfg_smtp_encryption':
			$cfg_smtp_encryption = !empty($row['value']) ? $row['value'] : '';
			break;

		case 'cfg_auto_approve_listing':
			$cfg_auto_approve_listing = !empty($row['value']) ? 1 : 0;
			break;

		case 'cfg_enable_sitemaps':
			$cfg_enable_sitemaps = !empty($row['value']) ? 1 : 0;
			break;

		case 'cfg_enable_reviews':
			$cfg_enable_reviews = $row['value'] == 0 ? 0 : 1;
			break;

		case 'cfg_enable_coupons':
			$cfg_enable_coupons = $row['value'] == 0 ? 0 : 1;
			break;

		case 'cfg_show_country_calling_code':
			$cfg_show_country_calling_code = !empty($row['value']) ? 1 : 0;
			break;

		case 'stripe_min_unit_is_cent':
			$stripe_min_unit_is_cent = !empty($row['value']) ? 1 : 0;
			break;

		case 'cfg_gdpr_on':
			$cfg_gdpr_on = !empty($row['value']) ? 1 : 0;
			break;

		case 'use_disqus':
			$use_disqus = !empty($row['value']) ? 1 : 0;
			break;

		case 'cfg_cur_without_cents':
			$cfg_cur_without_cents = !empty($row['value']) ? 1 : 0;
			break;
	}
}
...
$cfg_smtp_encryption           = trim(e($cfg_smtp_encryption         ));
$cfg_show_country_calling_code = trim(e($cfg_show_country_calling_code));
$stripe_min_unit_is_cent       = trim(e($stripe_min_unit_is_cent     ));
$cfg_gdpr_on                   = trim(e($cfg_gdpr_on                 ));
$use_disqus                    = trim(e($use_disqus                  ));
...
// cfg_smtp_encryption
if(!in_array($cfg_smtp_encryption, array('tls', 'ssl', ''))) {
	$cfg_smtp_encryption = 'tls';
}
...
if(!empty($_SESSION['user_connected']) && !empty($_SESSION['userid']) && !empty($_SESSION['loggedin_token'])) {
	...
	$hybridauth_provider_name = !empty($row['hybridauth_provider_name']) ? $row['hybridauth_provider_name'] : '';
	$loggedin_token           = !empty($row['loggedin_token'          ]) ? $row['loggedin_token'          ] : '';
...
if($maintenance_mode == 1 && !$is_admin) {
	if(!(isset($route[1]) && $route[1] == 'sign-in') && !(isset($route[1]) && $route[1] == 'sign-out')) {
...
// These are the files that are not accessed via ajax and show as endpoints in the address bar
