 ...
 * @return array
 */
function show_cats($cats_grouped_by_parent, $parent_id, $checked_cats, $is_first = 0) {
	if($is_first) {
...
function cat_name_transl($cat_id, $user_lang, $form, $orig_str) {
	global $conn;
...
if(!function_exists('get_listing_link')) {
	function get_listing_link($place_id, $place_slug = '', $cat_id = '', $cat_slug = '', $city_id = '', $city_slug = '', $state_slug = '', $cfg_permalink_struct = 'listing') {
		...
		if(empty($place_id) || !(ctype_digit(strval($place_id)) || is_int($place_id))) {
