...
// page url
$page_url = "$baseurl/admin/reviews?sort=$sort&show=$show&page=";
...
if($total_rows > 0) {
	...
	// show pending
	$where = "WHERE r.status = 'pending' OR r.status = 'approved'";
	...
	$query = "SELECT
		...
		LEFT JOIN rel_place_cat rel ON rel.place_id = r.place_id AND rel.is_main = 1
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$this_pubdate    = !empty($row['pubdate'   ]) ? $row['pubdate'   ] : '2020-03-18';
